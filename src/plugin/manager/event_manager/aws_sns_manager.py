...
class AWSSNSManager(ParseManager):
    ...
    def parse(self, raw_data: dict) -> dict:
        ...
        _LOGGER.debug(f"[AWSSNSManager] parse : {event}")
    ...
    @staticmethod
    def request_subscription_confirm(confirm_url: str) -> None:
        r = requests.get(confirm_url)
        _LOGGER.debug(f"[request_subscription_confirm] {r.status_code}, {r.content}")